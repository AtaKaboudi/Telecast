...
let raw = {
  ...
  let initial = raw.items.map(element => { return ({ ...element.id, ...element.snippet }) })
console.log(initial)
export async function reducerSuggestions(state = initial , action) {
    if (action.type === 'UPDATE') {
        return await youtubeAPI.nextPage(action.payload) ||initial;
    }
